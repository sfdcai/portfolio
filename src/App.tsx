...
function App() {
  ...
  return (
    <main className="min-h-screen bg-background bg-[length:24px_24px] [background-image:radial-gradient(circle,hsl(var(--dot-grid))_1px,transparent_1px)]">
      ...
      <section id="experience" className="py-16 md:py-24 bg-muted/30" style={{ contentVisibility: 'auto', containIntrinsicSize: 'auto 2000px' }}>
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="space-y-16">
            {t.experience.items.map((job: any, idx: number) => (
              <AnimatedSection key={idx} delay={0.1 * idx}>
                <div className="border-l-2 border-primary/30 pl-6 ml-4 relative">
                  <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full bg-primary border-4 border-background" />
                  
                  <div className="flex flex-wrap items-baseline justify-between gap-2 mb-2">
                    <div>
                      <h3 className="text-xl font-bold font-display text-foreground">{job.role}</h3>
                      <p className="text-primary font-semibold text-lg">{job.company}</p>
                    </div>
                    <div className="text-right">
                      <span className="text-sm font-medium text-muted-foreground block">{job.period}</span>
                      <span className="text-xs text-muted-foreground block">{job.location}</span>
                    </div>
                  </div>

                  <p className="text-muted-foreground mb-4 whitespace-pre-line">{job.desc}</p>

                  {job.highlights && job.highlights.length > 0 && (
                    <ul className="space-y-2 mb-6 pl-4 list-disc text-sm text-muted-foreground">
                  ...
                  {job.testimonials && job.testimonials.length > 0 && (
                    <div className="space-y-4 mt-6">
                      {job.testimonials.map((test: any, tIdx: number) => (
                        <blockquote key={tIdx} className="p-4 rounded-xl bg-muted/20 border border-border/50 relative">
                          <p className="text-sm text-muted-foreground italic mb-3">"{test.quote}"</p>
                          <footer className="flex items-center justify-between">
                            <div>
                              <span className="text-sm font-semibold text-foreground block">{test.author}</span>
      ...
      <section id="projects" className="py-16 md:py-24" style={{ contentVisibility: 'auto', containIntrinsicSize: 'auto 1500px' }}>
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {t.projects.items.map((proj: any, idx: number) => (
              <AnimatedSection key={idx} delay={0.1 * idx}>
                <div className="h-full p-6 rounded-2xl bg-card border border-border hover:border-primary/30 transition-all duration-200 flex flex-col group hover:-translate-y-1 hover:shadow-lg hover:shadow-primary/5">
                  <div className="flex items-start justify-between mb-4">
                    <h3 className="font-display text-xl font-bold group-hover:text-primary transition-colors">{proj.title}</h3>
                    <span className="badge px-3 py-1 bg-primary/10 text-primary text-xs font-semibold rounded-full">{proj.badge}</span>
                  </div>
                  
                  <p className="text-sm text-muted-foreground mb-6 flex-grow">{proj.desc}</p>
                  
                  <div className="flex flex-wrap gap-2 mb-6">
                    {proj.tech.map((t: string, tIdx: number) => (
                      <span key={tIdx} className="px-2 py-1 rounded-md text-xs bg-muted text-muted-foreground">{t}</span>
                    ))}
                  </div>
      ...
      {/* R&D Labs Section */}
      <section id="labs" className="py-16 md:py-24 bg-muted/30" style={{ contentVisibility: 'auto', containIntrinsicSize: 'auto 1000px' }}>
        <div className="max-w-5xl mx-auto px-6">
          <AnimatedSection>
            <h2 className="font-display text-2xl font-semibold mb-8 flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center">
            ...
            <p className="text-muted-foreground mb-12 max-w-3xl">
              Virtualization environments and security border systems hosted locally for infrastructure orchestration testing and systems automation R&D.
            </p>
          </AnimatedSection>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <AnimatedSection delay={0.1}>
              <div className="p-6 rounded-2xl bg-card border border-border hover:border-primary/30 transition-all duration-200 flex flex-col group hover:-translate-y-1 hover:shadow-lg hover:shadow-primary/5 h-full">
                <h3 className="font-display text-xl font-bold group-hover:text-primary transition-colors mb-2">Network & Infrastructure Lab</h3>
                <p className="text-sm text-muted-foreground mb-6">
                  VLAN segmentations, firewall rules, and traffic filtering systems running on custom firewall appliances to isolate test benches.
                </p>
                <Link
                  to="/homelab-network"
                  className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:text-primary/80 transition-colors group/cta mt-auto"
            ...
            <AnimatedSection delay={0.2}>
              <div className="p-6 rounded-2xl bg-card border border-border hover:border-primary/30 transition-all duration-200 flex flex-col group hover:-translate-y-1 hover:shadow-lg hover:shadow-primary/5 h-full">
                <h3 className="font-display text-xl font-bold group-hover:text-primary transition-colors mb-2">Unified Monitoring System</h3>
                <p className="text-sm text-muted-foreground mb-6">
                  Grafana, Netdata, and custom agent-based metrics exporters tracking Proxmox hosts, virtual appliances, and home automation systems.
                </p>
                <Link
                  to="/monitoring"
                  className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:text-primary/80 transition-colors group/cta mt-auto"
      ...
      <section id="blog" className="py-16 md:py-24" style={{ contentVisibility: 'auto', containIntrinsicSize: 'auto 1000px' }}>
        <div className="max-w-5xl mx-auto px-6">
          <AnimatedSection>
            <h2 className="font-display text-2xl font-semibold mb-8 flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center">
            ...
            <p className="text-muted-foreground mb-12 max-w-3xl">
              Sharing architectural notes, design decisions, and engineering deep dives from my experience building enterprise systems.
            </p>
          </AnimatedSection>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <AnimatedSection delay={0.1}>
              <div className="p-6 rounded-2xl bg-card border border-border hover:border-primary/30 transition-all duration-200 flex flex-col group hover:-translate-y-1 hover:shadow-lg hover:shadow-primary/5 h-full">
                <span className="text-xs text-primary font-semibold mb-2">DevOps</span>
                <h3 className="font-display text-xl font-bold group-hover:text-primary transition-colors mb-2">Salesforce DevOps Best Practices</h3>
                <p className="text-sm text-muted-foreground mb-6">
                  Establishing modern release governance pipelines using Copado, Azure DevOps, version-driven package structures, and git-based workflows.
                </p>
                <Link
                  to="/salesforce-devops"
                  className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:text-primary/80 transition-colors group/cta mt-auto"
                >
                  <span className="px-4 py-2 rounded-lg bg-primary/10 border border-primary/30 group-hover/cta:bg-primary/20 group-hover/cta:border-primary/50 transition-all duration-200">
                    Read Article
            ...
            <AnimatedSection delay={0.2}>
              <div className="p-6 rounded-2xl bg-card border border-border hover:border-primary/30 transition-all duration-200 flex flex-col group hover:-translate-y-1 hover:shadow-lg hover:shadow-primary/5 h-full">
                <span className="text-xs text-primary font-semibold mb-2">Artificial Intelligence</span>
                <h3 className="font-display text-xl font-bold group-hover:text-primary transition-colors mb-2">The Rise of AI Agents in Enterprise Systems</h3>
                <p className="text-sm text-muted-foreground mb-6">
                  A deep dive into how LLM reasoning loops, tool-calling interfaces, and local model workflows integrate with enterprise data architectures.
                </p>
                <Link
                  to="/ai-agents-blog"
                  className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:text-primary/80 transition-colors group/cta mt-auto"
          ...
          <div className="grid md:grid-cols-2 gap-12">
            ...
            <div>
              ...
              <div className="space-y-4">
                {t.education.items.map((item: any, i: number) => (
                  <AnimatedSection key={i} delay={0.1 + i * 0.1}>
                    <div className="p-5 rounded-2xl bg-card border border-border hover:border-primary/30 transition-colors duration-200 group">
                      ...
                      {'testimonial' in item && item.testimonial && (
                        <a href="https://www.linkedin.com/in/salesforce-technical-architect/details/recommendations/" target="_blank" rel="noopener noreferrer" className="block group">
